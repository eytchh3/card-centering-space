...
def _detect_frame_by_edge_scanning(warped: np.ndarray) -> Optional[np.ndarray]:
    ...
    # Restrict measurements to border bands so interior artwork edges are ignored.
    y0, y1 = int(0.15 * h), int(0.85 * h)
    x0, x1 = int(0.15 * w), int(0.85 * w)

    vertical_band = blur[y0:y1, :]
    ...
    def _peak_in_band(profile: np.ndarray, lo: int, hi: int) -> Optional[int]:
        lo = int(max(0, min(lo, len(profile) - 2)))
        hi = int(max(lo + 1, min(hi, len(profile) - 1)))
        if hi <= lo:
            return None
    ...
    left = _peak_in_band(x_profile, 0, int(0.25 * w))
    right = _peak_in_band(x_profile, int(0.75 * w), w - 1)
    top = _peak_in_band(y_profile, 0, int(0.25 * h))
    bottom = _peak_in_band(y_profile, int(0.75 * h), h - 1)
